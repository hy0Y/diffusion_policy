...
실행한다. 각 실험은 전용 RTX 3090 24 GiB 2장을 사용하고 GPU당 3 episode를 처리한다. 세 실험은 서로 다른 GPU pair에서 같은 local/global batch로 동시에 학습한다.
...
CUDA_VISIBLE_DEVICES=2,3 accelerate launch \
  --multi_gpu --num_processes 2 --mixed_precision no --main_process_port 0 \
...
2-GPU에서는 한 global step이 6 episodes, 48 groups, 192 windows이고 정확히 75 step이 한 epoch이다. 450이 `2 × 3`으로 나누어지므로 epoch 끝 dummy가 없다.

## 2-GPU production 실행값

| 항목 | 값 |
|---|---|
| GPU | RTX 3090 24 GiB × 2/experiment |
| Precision | FP32; MVP에서는 AMP를 사용하지 않음 |
| Local batch | 3 episodes = 24 groups = 96 DiT windows / rank |
| Global batch | 6 episodes = 48 groups = 192 DiT windows / optimizer step |
| Epoch | 75 optimizer steps; 450 real episodes, dummy 없음 |
| Train / validation workers | rank당 4 / 2 |
| Gradient accumulation | 1 |
| LR / warmup | `1e-4` / 375 steps (50-epoch 3,750-step budget의 10%) |
