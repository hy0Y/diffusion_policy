...
class LerobotP1PlainDataset(LerobotDataset):
    """Original observation/action dataset restricted by a fixed P1 manifest."""
        ...
        **kwargs,
        ...
        if missing:
            ...
            unexpected = (
                missing
                if subset is None
                else sorted(set(missing) & subset)
            )
            if unexpected:
                raise ValueError(
        ...
        if len(self._trajectory_ids) == 0:
            raise ValueError(f"empty P1 dataset: {task_name}")


class LerobotP1PlainCotrainingDataset(LerobotCotrainingDataset):
    """P1 90/5/5 manifest loader returning only original DP obs/action batches."""
    ...
    def __init__(
        ...
        metadata_config: dict = {"percentile_mixing_method": "weighted_average"},
    ):
        ...
        if manifest.get("task_set") != task_set:
            raise ValueError(
                f"P1 manifest task_set must be {task_set}: "
                + repr(manifest.get("task_set"))
            )
        mixture_mode = "train" if split_role == "train" else "val"
        dataset_soup_list = copy.deepcopy(DATASET_SOUP_REGISTRY[dataset_soup])
        for ds_meta in dataset_soup_list:
            if not os.path.isabs(ds_meta["path"]):
                from robocasa.macros import DATASET_BASE_PATH
                ds_meta["path"] = os.path.join(DATASET_BASE_PATH, ds_meta["path"])
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        return dataset[self.to_global_index(dataset, trajectory_id, step)]

    def get_validation_dataset(self):
        if self.split_role == "validation":
            return self
        return LerobotP1PlainCotrainingDataset(**self._validation_init)
