...
class TrainP1E0OriginalDPWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        if "ckpt_path" in cfg.task and cfg.task.ckpt_path is not None:
            ...
            if cfg.training.resume:
                self.load_checkpoint(path=cfg.task.ckpt_path, include_keys=["global_step", "epoch"])
            else:
        ...
        # training loop
        # Checkpoints are written at the end of an epoch, immediately before
        # ``self.epoch`` is incremented. A resumed run must therefore start
        # at the following epoch; otherwise it repeats the completed epoch.
        start_epoch = self.epoch
        if cfg.training.resume and self.global_step > 0:
            # Asynchronous checkpoint writing can capture ``epoch`` before its
            # end-of-epoch increment. ``global_step`` is authoritative for
            # the next completed 5k-step epoch boundary.
            start_epoch = max(
                self.epoch + 1,
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(start_epoch, cfg.training.num_epochs):
                ...
                if cfg.training.sample_every is not None and (self.epoch % cfg.training.sample_every) == 0:
                    if accelerator.is_main_process:
                        with torch.no_grad():
...
def main(cfg):
    workspace = TrainP1E0OriginalDPWorkspace(cfg)
    workspace.run()
